...
static async displayPeopleProfessional() { // returns array of Timesheet Table
  ...
  static async login(email, password, res) {
    try {
      switch (process.env.USER_MANAGER) {
        case "COGNITO":
         return await Cognito.login({ email, password, res });

        case "AURORA":
          return await Aurora.login({ email, password });
  ...
  static async displayCurrentUser(session) {
    ...
    const data = await db.executeStatement(sql, [
      ...
      {
        name: "sme_tenant_id",
        value: { longValue: session_data.sme_tenant_id },
      },
    ]);
  ...
  static async updateUserInfo({
    ...
    const db = new DB();
    const encryptedPasswordPhrase = newPassword
    ...
    const executeStatementParam = {
      ...
      sme_timezone_id: {
        ...
        value: { longValue: sme_timezone_id },
      },
    };

    if (!newPassword) delete executeStatementParam.newPassword;

    const data = await db.executeStatement(
      updateSql,
      Object.values(executeStatementParam)
    );
    return true;
  }
  ...
  static async findBySession(session) {
    try {
      ...
      const sme_tenants_homepage =
        person_raw[Object.keys(People.fields()).length + SME_TENANTS_HOMEPAGE]
          ?.stringValue;
  ...
  static async all(session) {
    ...
    const peopleRaw = data.records;
    const people = peopleRaw.map(
      (person) => new People(fromAurora(person, People.fields()))
    );
    return people;
  }
  ...
  static async displayPeopleProfessional(session) {
    ...
    const people = peopleRaw.map(
      (person) =>
        new People(fromAurora(person, People.peopleProfessionalFields()))
    );
    return people;
  }

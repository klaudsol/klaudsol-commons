...
class Aurora {
  static async login({ email, password }) {
    ...
    const capabilities = rawCapabilities.records.map(
  ...
  static async updatePassword({ session, oldPassword, newPassword }) {
    ...
    if (!session) throw new RecordNotFound("Session not found.");
    ...
    const id = data.records.map(([{ longValue: people_id }]) => ({
      people_id,
    }))[0];
    ...
    if (sqlPass.records.length === 0) {
      throw new RecordNotFound("Incorrect username or password.");
    }
   ...
   WHERE id = :id`;
    ...
    await db.executeStatement(updateSql, executeStatementParam);
    return false;
  }
